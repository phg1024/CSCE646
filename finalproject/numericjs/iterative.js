...
numeric.kaczmarz = function kaczmarz(A, b, maxIters, residue) {
    var maxIters = maxIters || 1024;
    var residue = residue || 1e-6;

    var Kaczmarz = function( A, b, AA, m, n, dotVV, dotMV ) {
        // initialization
        var iters = 0;
        var converged = false;
        var rows = m;
        ...
        // initialize x
        var x = numeric.rep([cols], 0);

        var prob = numeric.rep([rows], 0);
        ...
        var choose = function( eps ) {
            for(var i=0;i<prob.length-1;i++) {
                if( eps >= prob[i] && eps <= prob[i+1] )
                    return i;
            }
            return 0;
        };
        ...
        while( !converged && iters < maxIters ) {
            ...
            var i = choose( eps );
            var Ai = AA[i];
            var Ainorm = dot2(Ai, Ai);
            var p = dotMV(A, x);
            x = axpy((b[i] - p[i]) / Ainorm * relax, Ai, x);
            r = axpy(-1, p, b);
    ...
    switch(A.format) {
        case 'full': {
            ...
            return Kaczmarz(A, b, A, sA[0], sA[1], numeric.dot, numeric.dot);
        }
        case 'ccs': {
            var sA = numeric.ccsDim(A);
            var AA = numeric.ccsGather(A);
            return Kaczmarz(A, b, AA, sA[0], sA[1], numeric.dot, numeric.ccsMV);
        }
        case 'crs': {
            throw 'Not supported!';
        }
